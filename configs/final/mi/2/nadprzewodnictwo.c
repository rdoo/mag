...
double potencjalChemicznyMetodaBisekcji(double poczatkowa_gestosc_elektronow, double* poczatkowa_delta) {
	...
	while (fabs(a - b) > prog_dokladnosci) {
		double x = (a + b) / 2;
		gestosc1 = gestoscElektronow(x, poczatkowa_delta);
		gestosc2 = gestoscElektronow(a, poczatkowa_delta);
	...
	fprintf(plik_gestosc, "%.2f %.20f %.20f\n", L/nm2au, gestosc1*nm2au*nm2au*nm2au, gestosc2*nm2au*nm2au*nm2au);
...
int main() {
	...
	plik_gestosc = fopen("dane/gestosc.txt", "w");
	...
	fprintf(plik_gestosc, "%.20f\n", poczatkowa_gestosc_elektronow*nm2au*nm2au*nm2au); // bez 1e21
